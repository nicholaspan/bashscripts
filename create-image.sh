...
# Will only execute if there is an AMI older than 5 days

if [ `wc -l /tmp/amiid.txt | cut -d' ' -f1` -gt "0" ] ; then

    image_id=`cat /tmp/amiid.txt`
    lines=`wc -l /tmp/amiinfo.txt | cut -d' ' -f1`

    # Will only execute if the image is found
    if [ "$lines" -gt "0" ] ; then
        ...
        echo "There are no current images of $1 older than 5 days"
fi
